...
# Description: resample a 3D image to match the geometry of a reference image
# Expected input:
#   image           = SimpleITK.Image; input 3D image to resample
#   reference_image = SimpleITK.Image; image defining the output spacing
#   interpolator    = SimpleITK interpolator mode constant
# Output: SimpleITK.Image
...
def run_cleaning_workflow(input_image, out_dir):
    ...
    # Load native CT
    ct = sitk.ReadImage(input_image, sitk.sitkFloat32)
    print("Native size:", ct.GetSize())
    print("Native spacing:", ct.GetSpacing())

    # Run TotalSegmentator on the native CT
    totalsegmentator(input_image, out_dir, task="total", roi_subset=["hip_right"], fast=False, ml=False, device="gpu", skip_saving=False)
    totalsegmentator(input_image, out_dir, task="total", roi_subset=["hip_left"], fast=False, ml=False, device="gpu", skip_saving=False)

    # Load native TotalSegmentator masks
    right_mask = sitk.Cast(sitk.ReadImage(os.path.join(out_dir, "hip_right.nii.gz")) > 0, sitk.sitkUInt8)
    left_mask = sitk.Cast(sitk.ReadImage(os.path.join(out_dir, "hip_left.nii.gz")) > 0, sitk.sitkUInt8)

    # Create 1.5-mm LPS working CT
    ct_lps = sitk.DICOMOrient(ct, "LPS")
    if check_spacing(ct_lps.GetSpacing(), target=1.5, tol=1e-2):
        working_ct = ct_lps
        print("CT is already 1.5-mm isotropic; skipping resampling.")
    else:
        working_ct = resample_to_isotropic(ct_lps, target_spacing=1.5, interpolator=sitk.sitkLinear)

    # Resample native masks to working CT geometry
    right_working = resample_to_reference(right_mask, working_ct, interpolator=sitk.sitkNearestNeighbor)
    left_working = resample_to_reference(left_mask, working_ct, interpolator=sitk.sitkNearestNeighbor)
    print("Working size:", working_ct.GetSize())
    print("Working spacing:", working_ct.GetSpacing())

    # Convert working images to NumPy arrays in (z, y, x)
    ct_arr = sitk.GetArrayFromImage(working_ct)
    right_hip_pred_arr_rev = sitk.GetArrayFromImage(right_working)
    left_hip_pred_arr_rev = sitk.GetArrayFromImage(left_working)
    right_hip_pred_arr_rev_bi = (right_hip_pred_arr_rev > 0).astype(np.uint8)
    left_hip_pred_arr_rev_bi = (left_hip_pred_arr_rev > 0).astype(np.uint8)
    right_hip_cropped = np.where(right_hip_pred_arr_rev_bi > 0, ct_arr, np.nan)
    left_hip_cropped = np.where(left_hip_pred_arr_rev_bi > 0, ct_arr, np.nan)
    ...
    # Convert cleaned working arrays to SimpleITK images
    right_cleaned_working = sitk.GetImageFromArray(right_hip_mask_cleaned_final.astype(np.uint8))
    left_cleaned_working = sitk.GetImageFromArray(left_hip_mask_cleaned_final.astype(np.uint8))

    # Assign 1.5-mm working geometry
    right_cleaned_working.CopyInformation(working_ct)
    left_cleaned_working.CopyInformation(working_ct)

    # Return cleaned masks to native CT geometry
    right_cleaned_native = resample_to_reference(right_cleaned_working, ct, interpolator=sitk.sitkNearestNeighbor)
    left_cleaned_native = resample_to_reference(left_cleaned_working, ct, interpolator=sitk.sitkNearestNeighbor)

    # Ensure binary output
    right_cleaned_native = sitk.Cast(right_cleaned_native > 0, sitk.sitkUInt8)
    left_cleaned_native = sitk.Cast(left_cleaned_native > 0, sitk.sitkUInt8)

    # Save final cleaned masks
    sitk.WriteImage(right_cleaned_native, os.path.join(out_dir, "hip_right_cleaned.nii.gz"))
    sitk.WriteImage(left_cleaned_native, os.path.join(out_dir, "hip_left_cleaned.nii.gz"))
